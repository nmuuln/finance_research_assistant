...
# Usage: ./build-and-push-do.sh [registry-name]
...
echo -e "${GREEN}========================================${NC}"
echo -e "${GREEN}Build & Push to DigitalOcean Registry${NC}"
echo -e "${GREEN}========================================${NC}"
echo ""
...
docker build --platform linux/amd64 -t "${IMAGE_NAME}:${VERSION}" -f Dockerfile.adk-web .
...
echo ""
echo "1. Deploy to App Platform:"
echo "   doctl apps create --spec .do/app-from-registry.yaml"
echo ""
echo "2. Or deploy to a Droplet:"
echo "   ssh root@YOUR_DROPLET_IP"
echo "   doctl registry login"
echo "   docker run -d -p 80:8080 --env-file .env $FULL_IMAGE_NAME"
echo ""
